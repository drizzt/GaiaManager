...
// SHOULD DETERMINE USB #?
						...
						if (chg_dst_hdd) {
							if (stat("/dev_usb", &s) < 0) {
								...
								open_table.entries[0].compare_addr =
									((uint32_t) & open_table.arena[0]) - ((uint32_t) & open_table) + dest_table_addr;

								open_table.entries[0].replace_addr =
									((uint32_t) & open_table.arena[0x800]) - ((uint32_t) & open_table) +
									dest_table_addr;
		...
		if (mode_list == GAME)
			draw_device_list((fdevices |
							 ...
							 payload_type, direct_boot, ftp_flags & 2, chg_dst_hdd);
		else
